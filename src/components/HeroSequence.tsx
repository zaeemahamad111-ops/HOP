  ...
  return (
    <div ref={wrapRef} style={{ height: "500vh" }} className="relative">
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-[#06050a]">
        ...
        {SCENES.map((scene, i) => (
          <div key={i} ref={el => { sceneRefs.current[i] = el; }} className="absolute left-0 top-0 bottom-0 w-full md:w-1/2 flex flex-col justify-center px-10 md:px-24 pointer-events-none" style={{ opacity: 0 }}>
            ...
            <h1 className="font-display leading-[0.9] mb-4 text-ivory" style={{ fontSize: "clamp(2.5rem, 6vw, 6.5rem)", letterSpacing: "-0.03em" }}>
              <span className="block">{scene.line1}</span>
              <em className="not-italic block" style={{ color: GOLD }}>{scene.line2}</em>
            </h1>
            <p className="font-accent italic leading-relaxed text-ivory/40 max-w-xl" style={{ fontSize: "clamp(0.9rem, 1.3vw, 1.1rem)" }}>{scene.sub}</p>
          </div>
        ))}
